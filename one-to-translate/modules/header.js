...
function Button(props) {
    return <div className='m-auto'>
        <div className={'p-1 mx-4 sm:text-base text-sm duration-300 px-2 flex ' + (props.sel ? 'font-bold text-b-m':'font-light text-b-d')}>
...
export default function Header(props) {
    ...
    return <div className='w-full'>
        <div className={'fixed z-50 w-screen duration-300 ease-in-out drop-shadow-lg bg-white ' + (transparent ? '':'')}>
            <div className='m-auto flex sm:p-10 p-4 sm:py-2 py-2 max-w-7xl'>
                <div className={'ease-in-out transition-all header-title duration-300 pl-4 flex text-b-d sm:text-3xl text-lg' + (transparent ? '':'')}>
                    <div className='w-full'>
                        <Link href='/'><div className='m-auto header-title flex w-fit'>
                            <div className='my-auto'>OneToTranslate</div>
                        </div></Link>
                    </div>
                </div>
                ...
                <div className='m-auto'><Button text='Home' route='/' sel={props.page == 1} t={transparent} /> </div>
                    ...
                    <div className='m-auto relative'><Button text={<div onMouseOver={() => setHoverDD1(true)} onMouseLeave={() => setHoverDD1(false)}>Services</div>} route='/services' sel={props.page >= 4} t={transparent} alt={
                        <div className=''>
                            ...
                            <div className={'absolute w-[16rem] text-b-d overflow-hidden transition-height duration-300 max-w-[12rem] -translate-x-1/2 left-1/2 mt-2' + (DD ? ' h-[16rem]': ' h-0')} onMouseOver={() => setHoverDD2(true)} onMouseLeave={() => setHoverDD2(false)}>
                                <div className='p-2 bg-white sm:text-base text-sm text-p font-light'>
                                    <div className={'p-2 hover:pl-4 hover:pr-0 duration-300' + (props.page == 5 ? ' font-extrabold text-b-m':'')}><Link href={'/services/' + to_link(services[0].title)}>{services[0].title}</Link></div>
                                    <div className={'p-2 hover:pl-4 hover:pr-0 duration-300' + (props.page == 6 ? ' font-extrabold text-b-m':'')}><Link href={'/services/' + to_link(services[1].title)}>{services[1].title}</Link></div>
                                    <div className={'p-2 hover:pl-4 hover:pr-0 duration-300' + (props.page == 7 ? ' font-extrabold text-b-m':'')}><Link href={'/services/' + to_link(services[2].title)}>{services[2].title}</Link></div>
                                </div>
                            </div>
                        </div>
                    } /> </div>
        ...
        <div className={props.page == 1 ? '':'sm:h-[3.3rem] h-[2.7rem]'} />
    </div>;
}