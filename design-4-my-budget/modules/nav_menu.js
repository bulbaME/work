        ...
        <div className={'absolute transition-height duration-300 overflow-hidden max-w-[16rem] -ml-[13rem] sm:mt-[1.1rem] mt-[1.1rem]' + (dropDown ? ' h-[30rem]':' h-0')}>
            <div className='px-4 py-3 bg-white rounded-b-2xl'>
                ...
                <div className={'ml-4 overflow-hidden transition-height duration-300 -mb-2 ' + (dropDown2 ? 'h-[11rem]':'h-0')}>
                    <div className={'my-2 py-0.5 w-fit rounded-full ' + (props.page == 5 ? 'underline decoration-2 decoration-m-l font-bold':'')}><Link href={services[0].link}>{services[0].title}</Link></div>
                    <div className={'my-2 py-0.5 w-fit rounded-full ' + (props.page == 6 ? 'underline decoration-2 decoration-m-l font-bold':'')}><Link href={services[1].link}>{services[1].title}</Link></div>
                    <div className={'my-2 py-0.5 w-fit rounded-full ' + (props.page == 7 ? 'underline decoration-2 decoration-m-l font-bold':'')}><Link href={services[2].link}>{services[2].title}</Link></div>
                </div>
            </div>
        </div>
    </div>;
