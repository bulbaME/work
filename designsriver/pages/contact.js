            ...
            <div className='flex flex-wrap-reverse rounded-[2rem] bg-white drop-shadow-lg w-full'>
                ...
                <form onSubmit={() => send_form(name, mail, product, text)} className='text-black h-fit p-4'>
                    ...
                    <label className='m-4 sm:flex text-b'>
                        ...
                        <input required className='bg-stone-100 rounded-2xl select-none appearance-none outline-none text-p-d p-2 px-4 mr-2 sm:w-3/5 w-full m-auto' value={name} onChange={(e) => setName(e.target.value)} />
                    </label>
                    <label className='m-4 sm:flex text-b'>
                        <div className='m-2 mr-0 sm:text-xl font-semibold'>
                            Your E-mail
                        </div>
                        <input required type='email' className='bg-stone-100 rounded-2xl select-none appearance-none outline-none text-p-d p-2 px-4 mr-2 sm:w-3/5 w-full m-auto' value={mail} onChange={(e) => setMail(e.target.value)} />
                    </label>
                    <label className='m-4 sm:flex text-b'>
                        <div className='m-2 mr-0 sm:w-1/3 sm:text-xl font-semibold'>
                            Describe your needs
                        </div>
                        <textarea className='bg-stone-100 select-none rounded-2xl appearance-none outline-none text-p-d p-2 px-4 h-[6rem] mr-2 sm:w-3/5 w-full m-auto resize-none' value={text} onChange={(e) => setText(e.target.value)} />
                    </label>
