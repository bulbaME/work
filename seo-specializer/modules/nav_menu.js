...
export default function NavMenu(props) {
    ...
    return <div>
        ...
        <div className={'absolute transition-height duration-300 overflow-hidden -ml-[10rem] sm:mt-[1.15rem]' + (dropDown ? ' h-[30rem]':' h-0')}>
            <div className='p-4 bg-black text-white text-sm'>
                <div className={'my-3 ' + (props.page == 1 ? 'text-g-m font-bold':'')}><Link href='/'>Home</Link></div>
                <div className={'my-3 ' + (props.page == 2 ? 'text-g-m font-bold':'')}><Link href='/about'>About</Link></div>
                <div className={'my-3 ' + (props.page == 3 ? 'text-g-m font-bold':'')}><Link href='/contact'>Contact</Link></div>
