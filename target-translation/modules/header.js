...
function Button(props) {
    return <div className={'m-auto w-fit px-3 flex py-2' + props.sel}>
        <Link href={props.route}><div className='font-extrabold text-xl'>{props.text}</div></Link>
        {props.alt}
    </div>;
}
...
export default function Header(props) {
    ...
    const updateT = (scroll) => {
        if (scroll < 100 && props.page <= 4) setTransparent(true);
    ...
    return <div>
        ...
        { props.page > 4 || props.page <= 0 ? <div className='pt-[6.5rem] sm:pt-[4rem]' />:''}
    </div>;
}